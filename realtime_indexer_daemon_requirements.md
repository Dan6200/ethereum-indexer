...
- **F1.2 Checkpoint Reading:** Must read the last known good state (block_number, block_hash) from the PostgreSQL Checkpoint Table upon startup and before processing each new block.
...
- **F2.1 Detection Logic:** Before processing a new block, the Daemon must compare the incoming block's parentHash with the previously stored block_hash in the Checkpoint Table.
...
- **NFR2.1 Key Metrics:** Must expose core metrics (e.g., Prometheus/Grafana) including: latest_indexed_block_number, indexing_latency_seconds, and reorgs_detected_total.
