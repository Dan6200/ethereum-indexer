...
- **F2.2 Checkpoint Update:** Must atomically update the Checkpoint table's block_number and block_hash to the state preceding the rollback.
...
- **NFR2.1 Indexed Rollback:** The core data tables in PostgreSQL must be indexed on the block_number column (e.g., idx_block_number in indexer_postgres_schema.md) to ensure the DELETE operation is near-instantaneous.
